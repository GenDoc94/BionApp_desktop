...
import { buildChipPanels, chipPanelMatchesQuery, filterChipPanels, } from "./chipPageData";
var chips = [
...
describe("chipPageData search", function () {
    var panels = buildChipPanels(chips, asignaciones);
    it("matches by chip number without false positives in chip names", function () {
    ...
    it("matches by chip name (substring, case insensitive)", function () {
    ...
    it("matches by sample number in any flowcell", function () {
    ...
    it("returns all panels when query is empty", function () {
        expect(filterChipPanels(panels, "")).toHaveLength(2);
        expect(filterChipPanels(panels, "   ")).toHaveLength(2);
    });
    it("filters panels by query", function () {
