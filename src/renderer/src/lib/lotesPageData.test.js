...
import { buildLotesHighlightPath, filterLots, findLotId, hydrateLecturasMarcadoFromLots, hydrateMuestrasFromLots, lotLnForDisplay, groupUsosExtraido, groupUsosLm, lotExpFromInputValue, lotExpToInputValue, lotOptionLabel, parseLotesHighlight, resolveHighlightedLotId, sortLots, } from "./lotesPageData";
var lots = [
...
describe("lotesPageData", function () {
    it("sorts by LN descending", function () {
        expect(sortLots(lots).map(function (l) { return l.id; })).toEqual([1, 2, 3]);
    });
    it("adds Exp to the label when LN is shared", function () {
        expect(lotOptionLabel(lots[0], lots)).toBe("250428048 · 06/08/2026");
        expect(lotOptionLabel(lots[2], lots)).toBe("240515028");
    });
    it("finds lot by id or triple", function () {
    ...
    it("shows LN from catalog when the sample only has Id_LtE", function () {
    ...
    it("hydrates PN/LN/Exp from lot ids", function () {
        var muestras = [{ NumBN: 1, Id_LtE: 3 }, { NumBN: 2 }];
    ...
    it("parses highlight query", function () {
        ...
        expect(buildLotesHighlightPath({ tipo: "membrana", id: 4 })).toBe("/calidad?tipo=membrana&id=4&tab=lotes");
    });
    it("resolves highlight by ln fallback", function () {
        expect(resolveHighlightedLotId(lots, { tipo: "extraido", ln: "240515028" })).toBe(3);
    });
    it("converts Exp between catalog and date input", function () {
    ...
    it("groups usages and filters by BN", function () {
        var _a;
        var extra = groupUsosExtraido([
            { Id_LtE: 3, NumBN: 99 },
            { Id_LtE: 3, NumBN: 5 },
        ]);
        expect((_a = extra.get(3)) === null || _a === void 0 ? void 0 : _a.map(function (u) { return u.NumBN; })).toEqual([5, 99]);
        var lm = groupUsosLm([
            { lotId: 1, NumBN: 72, NumLectura: 2, NumLectMarc: 1 },
        ]);
        expect(filterLots(lots, extra, lm, "99").map(function (l) { return l.id; })).toEqual([3]);
    });
});
